...
for i in range(0, 1):
    ...
    '''
    file_path = caminho('circles-noisy=0.5', i)
    stream_generator = CIRCLES(concept_length=2000, random_seed=i, noise_rate=0.05)
    stream_generator.generate(file_path)
    
    file_path = caminho('sine2-noisy=0.5', i)
    stream_generator = SINE2(concept_length=2000, random_seed=i, noise_rate=0.05)
    stream_generator.generate(file_path)
