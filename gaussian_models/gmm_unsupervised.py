...
class PlotGMM():
    ...
    def animation(self, it):
        ...
        def update(i):
            ...
            indexColors = [np.argmax(matrixWeights[j]) for j in range(len(matrixWeights))]
...
class GMM(PlotGMM):
    ...
    def fitClustering(self, train_input, K):
        ...
        # if there is only one gaussian, we calculate over all observations
        if(K == 1):
            # mean
            mu = np.mean(train_input, axis=0)
            # cov
            cov = np.cov(np.transpose(train_input))
        ...
        # if there is more than one gaussian, we initilize the parameters randomly
        else:
            # allocating itens for each gaussian
            for _ in range(K):
    
                # selecting randomly one observation to start the EM process
                if(len(self.train_input) > 20):
                    # selecting random examples to create the initial parameters
                    indexes = [np.random.randint(0, len(self.train_input)-1) for _ in range(10)]
                    observation = [self.train_input[i] for i in indexes]
                    # mean
                    mu = np.mean(observation, axis=0)
                    # cov
                    cov = np.cov(np.transpose(observation))
                    
                    
                # dealing with few observations
                else:
                    # getting only one observation
    ...
    def trainEM(self, iterations, criterion=True):
        ...
        if(self.n > 1):
            ...
            for i in range(iterations):
                ...
                # storing the best gaussian
                self.BestLogLike(self.listLoglike)
                
                # stop criterion
                if(criterion and (i > 2)):
                    calculo = 100-(self.listLoglike[i]*100/self.listLoglike[i-1])
                    if(calculo < 0.15):
                        # receiveing the best gaussians
                        self.gaussians = copy.deepcopy(self.bestGaussians)
                        break
    ...
    def BestLogLike(self, listLoglike):
        '''
        Method to store the best gaussians according to the loglikelihood function over time
        :listLoglike: list with the loglikelihood values over iterations
        '''
        
        # storing the best gaussians in the first execution
        if(len(listLoglike) == 1):
            self.bestGaussians = copy.deepcopy(self.gaussians)
            self.bestLog = listLoglike[0]
            
        # analyzing each iteration to store the best gaussian 
        elif(len(listLoglike) > 1 and listLoglike[-1] < self.bestLog):
            self.bestGaussians = copy.deepcopy(self.gaussians)
            self.bestLog = listLoglike[-1]
...
def main():
    ...
    gmm.trainEM(50, False)
    #gmm.animation(150)
    
    plt.plot(gmm.listLoglike)
    plt.show()
    
    #for gaussians in gmm.gaussians:
        #print(gaussians.mu)
        #print(gaussians.sigma)
        #print("---------------")
